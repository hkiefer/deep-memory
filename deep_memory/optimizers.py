...
class Rprop():
    def __init__(self, learning_rate = 0.01, etapos = 1.2, etaneg = 0.5):
        self.learning_rate = learning_rate
        self.w_updt = None
        self.etapos = etapos
        self.etaneg = etaneg
    def update(self, w, grad_wrt_w, old_grad, old_eta):
        # If not initialized
        if self.w_updt is None:
            self.w_updt = np.zeros(np.shape(w))
        if np.any(grad_wrt_w.T.dot(old_grad) > 0):
            eta_new = self.etapos*old_eta
            
        elif np.any(grad_wrt_w.T.dot(old_grad) < 0):
            eta_new = self.etaneg*old_eta
            
        else:
            eta_new = old_eta
        
        self.w_updt = grad_wrt_w
        return eta_new, w - eta_new * self.w_updt
